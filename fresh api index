  ...
  <script>
    var app = {
      ...
      printLabels: function() {
        ...
        var html = '<html><head><style>body{font-family:Arial;margin:20px}.label{width:' + labelDim + ';height:' + heightDim + ';border:2px solid #000;padding:15px;margin-bottom:10px;page-break-inside:avoid;box-sizing:border-box}.label-header{font-size:10px;font-weight:bold;margin-bottom:8px}.label-product{font-size:14px;font-weight:bold;margin:8px 0}.label-address{font-size:11px;line-height:1.4;margin:5px 0;border-top:1px solid #000;border-bottom:1px solid #000;padding:5px 0}.label-count{font-size:20px;font-weight:bold;text-align:center;margin-top:8px}</style></head><body>';
        dayOrders.forEach(function(order) {
          if (order.items) {
            order.items.forEach(function(item, idx) {
              html += '<div class="label"><div class="label-header">BASKETS BY BLIMI</div><div class="label-product">' + item.product + '</div><div class="label-address">' + order.address + '</div><div class="label-count">' + (idx + 1) + ' of ' + order.items.length + '</div></div>';
            });
          }
        });
        html += '</body></html>';
      ...
      render: function() {
        ...
        if (dayOrders.length === 0) {
          ...
          dayOrders.forEach(function(order, orderIdx) {
            html += '<div class="order-item"><div style="display:flex;justify-content:space-between;align-items:start"><div><div class="order-name">' + order.lastName + '</div>';
            if (order.deliverTo) html += '<div class="order-address">' + order.deliverTo + '</div>';
            html += '<div class="order-address">' + order.address + '</div><div style="font-size:11px;color:#9ca3af;margin-top:5px;text-transform:uppercase">' + (order.pickupOrDelivery || 'delivery') + '</div></div><button onclick="app.printOrder(' + orderIdx + ')" style="background:#16a34a;color:white;border:none;padding:8px 12px;border-radius:4px;cursor:pointer;font-size:12px;white-space:nowrap;margin-left:10px;height:fit-content">🖨️ Print</button></div>';
